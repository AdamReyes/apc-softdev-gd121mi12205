...
public class GameManager : MonoBehaviour {
    ...
    List<QuizData> quizList; //QuizData.cs
    int quizTotal;
    int quizIndex = 0;
    ...
    void DrawQuiz() //Start of drawing the questions
    {
        HideQuiz();
    ...
    void QuizInit() //Quiz creator Related to QuizData.cs
    {
        quizList = new List<QuizData>();
        List<string> champs = new List<string>(); //new array Champions
        string[,] dic = LolSkillData.dic; //Dictionary
    ...
    void InitGame() //Start of the quiz
    {
        friendHpMan.InitHp();
        enemyHpMan.InitHp();
    ...
    void ShowQuiz() //GUI of questions shown
    {
    ...
    void ClickAnswer(int no) // checker if right or wrong
    {
        if (!quizOn) return;
        quizOn = false; //the quiz stops because this part checks the answer
        QuizData item = quizList[quizIndex];
        if (item.correct == no) 
        {
        ...
        {
            DrawQuiz(); //Draw another question
        }));
    }

    public void OnClickAnswer1()
    {
        ClickAnswer(0); //Answer
    }
    public void OnClickAnswer2()
    {
        ClickAnswer(1); //Answer
    }
    public void OnClickAnswer3()
    {
        ClickAnswer(2); //Answer
    }
    public void OnClickAnswer4()
    {
        ClickAnswer(3); //Answer
    }
